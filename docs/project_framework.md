...
- 启动和回收环境、飞控子进程。
- 写出 `campaign_summary.json`，并聚合成功实例的命中、最近点和故障统计。
...
以完整规划为目标，当前工程功能完成度约为 **80%**；以“可构建、可运行、
可命中的最小双进程闭环”为目标，核心 MVP 已完成。

## 12. 建议的后续顺序

1. 为飞控增加独立静态库和单元测试，再实现调度器、状态机、估计器和保护。
2. 让 `instance_manager` 读取 `instances[]`，传递逐实例配置和随机种子。
3. 为 `instance_manager` 增加端口预检、就绪握手和失败退出码。
4. 扩展更完整的故障类型库和真实 DEM 接入。
5. 实现二进制日志回放、日志转换、地图预处理和批量统计工具。
...
| 飞控状态机/调度器 | 仅设计预留 | 配置和头文件有结构，主循环尚未按任务调度执行 |
| 自动驾驶仪/控制分配 | 仅设计预留 | 当前控制仍是加速度级虚拟接口，不是真实舵面闭环 |
| UDP 闭环通信 | 已进入主链路 | 固定小端线协议、CRC、版本和 instance_id 校验已接入 |
| 共享内存通信 | 未使用 | 当前设计选择 UDP；共享内存可作为后续接口扩展 |
| 多实例独立运行 | 部分进入主链路 | 管理器可启动独立进程对；尚未读取逐实例配置和显式种子 |
